...
subtitle: "Pobreza Multidimensional"
output: 
  flexdashboard::flex_dashboard:
    navbar:
    - {title: "Ver en Redatam", href: " http://aru.noip.me/", align: right}  
    - {icon: "fa-info-circle", href: "https://www.aru.org.bo/", align: right}  
    - {icon: "fa-facebook", href: "https://www.facebook.com/FundacionARU", align: right}
...
minsdim<-bd %>% group_by(s0_1) %>% summarise_at(vars(sdim_1:sdim_35),min,na.rm=T)
maxsdim<-bd %>% group_by(s0_1) %>% summarise_at(vars(sdim_1:sdim_35),max,na.rm=T)
...
fig <- fig %>%
  layout(margin=mcom,legend = list(orientation = 'h',x = 0, y = -0.7,font = list(size = 10)),
    polar = list(
      radialaxis = list(
        visible = T,
        range = c(0,100)
...
# AIO Uru Chipaya {data-navmenu="Municipio" data-icon="fa-list"}
